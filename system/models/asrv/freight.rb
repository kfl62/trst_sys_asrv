...
module Asrv
  class Freight < Trst::Freight
    ...
    has_many    :ins,         class_name: "Asrv::FreightIn",            inverse_of: :freight
    has_many    :outs,        class_name: "Asrv::FreightOut",           inverse_of: :freight
    has_many    :stks,        class_name: "Asrv::FreightStock",         inverse_of: :freight
    ...
    class << self
      ...
      def ins
        ids = all.pluck(:id)
        Asrv::FreightIn.where(:freight_id.in => ids)
      end
      # @todo
      def outs
        ids = all.pluck(:id)
        Asrv::FreightOut.where(:freight_id.in => ids)
      end
      # @todo
      def stks
        ids = all.pluck(:id)
        Asrv::FreightStock.where(:freight_id.in => ids)
      end
    end # Class methods
    ...
    def order_csn
      if csn.count > 1
        ...
        h_all = Hash[csn.map{|o| o.unshift(Asrv::PartnerFirm.find(o[0]).name[0])}.sort.map{|o| o.shift;o}]
