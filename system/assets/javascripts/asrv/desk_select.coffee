    ...
    desk:
      select:
        unit: (node)->
          $select = node
          $select.change ()->
            if $select.val() is 'null' then Asrv.unit_info.update() else Asrv.unit_info.update($select.find('option:selected').text())
        ...
        select2: (node)->
          ...
          $.extend true, $.fn.select2.defaults,
            formatInputTooShort: (input, min)->
              Asrv.desk.select.inputTooShortMsg(input, min)
            formatSearching: ()->
        ...
        inputTooShortMsg: (input, min)->
          ...
          $msg
        init: () ->
          $('select.asrv, input.select2').each ()->
            $select =  $(@)
