    ...
    $.extend(true, Asrv, {
      desk: {
        ...
        idPnHandle: function() {
          ...
          if (Asrv.desk.idPnValidate($input.val())) {
        ...
        init: function() {
          ...
          if ((_ref = Trst.desk.hdo.dialog) === 'create' || _ref === 'edit') {
            if ($('input[name*="id_pn"]').length) {
              Asrv.desk.idPnHandle();
              $('input[name*="id_pn"]').on('keyup', function() {
                return Asrv.desk.idPnHandle();
              });
            }
            $('.focus').focus();
          } else {
          ...
          if ($('table.scroll').height() > 450) {
            Asrv.desk.scrollHeader($('table.scroll'));
          }
          $log('Asrv.desk.init() Ok...');
          if ($('select.asrv, input.select2').length) {
            require(['asrv/desk_select'], function(select) {
              return select.init();
            });
          }
          if ($ext = Trst.desk.hdo.js_ext) {
            require(["asrv/" + $ext], function(ext) {
              return ext.init();
            });
          }
        }
