(function() {
  define(['/javascripts/libs/select2.min.js', '/javascripts/libs/jquery.ui.datepicker-ro.js', 'asrv/desk'], function() {
    $.extend(Asrv, {
      unit_info: {
        node: function() {
        ...
        update: function(unit_name) {
          ...
          if ($('body').data('admin')) {
            if ($('#unit_info').length) {
              ...
              $('#xhr_tasks ul li').last().append(Asrv.unit_info.node());
            }
          }
          return Asrv.unit_info_text;
        }
      },
      init: function() {
        ...
        $('nav.menu ul li a').filter('[id^="page"]').click(function() {
          ...
          $('#xhr_tasks').load("/sys/tasks/" + $page_id, function() {
            ...
            return Asrv.unit_info.update(Trst.lst.unit_info_txt);
          });
          return false;
        });
        if (Trst.lst.page_id) {
